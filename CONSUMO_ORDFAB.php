...
//			if ($px_pfp->cmov_lq=="PF" && $px_pfp->esta_lq==500 && $px_pfp->auno_lq!="" and $pend->consumo==false){
			if ($px_pfp->cmov_lq=="PF" && $px_pfp->esta_lq==500 && $px_pfp->auco_lq!="" and $pend->consumo==false){
				...
				foreach ($sl_tar_lista as $tarx){
					$tarea = $tarx;
					$serv->set_subestado("proceso de ZRFCPP_CONSUMO_ORDFAB>>$tarea->nmov_lt $tarea->declarado_en_sap");
					if($tarea->declarado_en_sap=="Notificada"){
							continue;
					}
					if ($tarea->declarado_en_sap=="Particionada"){
						$mat = new Material();
     					$serv->set_subestado("Inicio proceso Buscar Extendido >>$tarea->nmov_lt $tarea->ndoc_lt $px_pfp->cmov_lq,$px_pfp->nmov_lq");
						
						$mat = $mat->buscarExtendido($tarea->nroMaterialSap());						
     					$serv->set_subestado("Fin proceso Buscar Extendido >>$tarea->nmov_lt ");
				        ...
				        $serv->set_subestado("Inicio procesa consumos ZRFCPP_CONSUMO_ORDFAB $tarea->nmov_lt");
