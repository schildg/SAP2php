...
//	saprfc_table_append ($fce,"MATERIAL_RA", array ("SIGN"=>"I","OPTION"=>"BT","LOW"=>"000000000800000226","HIGH"=>"000000000800000226"));
	...
	saprfc_table_append ($fce,"MOVE_TYPE_RA", array ("SIGN"=>"I","OPTION"=>"BT","LOW"=>"101","HIGH"=>"102"));
	...
	saprfc_table_append ($fce,"STGE_LOC_RA", array ("SIGN"=>"I","OPTION"=>"EQ","LOW"=>"PRFE","HIGH"=>"PRFE"));
...
/*	foreach ($cabecera_mov_mat as $exp){
		echo "cabeeras;"."MAT_DOC=>".$exp[MAT_DOC]."DOC_YEAR=>".$exp[DOC_YEAR]."TR_EV_TYPE=>".$exp[TR_EV_TYPE]."PSTNG_DATE=>".$exp[PSTNG_DATE]."REF_DOC_NO=>".$exp[REF_DOC_NO]."VBELN=>".$exp[VBELN]."WERKS=>".$exp[WERKS]."LGORT=>".$exp[LGORT]."WDATU=>".$exp[WDATU].";\r\n";
	}
	
/*	foreach ($cabecera_mov_mat as $cabecera){
		echo "DOCMAT_HU;st-doc;".$cabecera[MAT_DOC].";".$cabecera[DOC_YEAR].";".$cabecera[TR_EV_TYPE].";".$cabecera[PSTNG_DATE].";".$cabecera[REF_DOC_NO].";\r\n";
	}
*/
...
/*	foreach ($linea_mov_mat as $exp){
		echo "lineas;"."MAT_DOC=>".$exp[MAT_DOC]."DOC_YEAR=>".$exp[DOC_YEAR]."VEMNG=>0"."VEMEH=>".$exp[VEMEH]."MATNR=>".$exp[MATNR].";\r\n";
	}
	...
	foreach ($cabecera_mov_mat as $cabecera){
		foreach ($linea_mov_mat as $linea){
			if($linea[MAT_DOC]==$cabecera[MAT_DOC] && $linea[DOC_YEAR]==$cabecera[DOC_YEAR]){
				echo "DOCMAT_HU;st-lar;".";".$cabecera[MAT_DOC].";".$cabecera[DOC_YEAR].";".$cabecera[PSTNG_DATE].";".$linea[MOVE_TYPE].";".$linea[MATNR].";".number_format($linea[VEMNG], 3, '.', '').";".$linea[VEMEH].";\r\n";
				foreach ($lote_mov_mat as $lote){
					if($lote[MAT_DOC]==$linea[MAT_DOC] && $lote[DOC_YEAR]==$linea[DOC_YEAR] && $lote[MATNR]==$linea[MATNR]){					
						echo "DOCMAT_HU;st-rld;".";".$lote[MATNR].";".$lote[CHARG].";".number_format($lote[VEMNG], 3, '.', '').";".$lote[VEMEH].";".$lote[VFDAT].";\r\n";
						foreach ($ut_mov_mat as $ut){
							if($ut[MAT_DOC]==$lote[MAT_DOC] && $ut[DOC_YEAR]==$lote[DOC_YEAR] && $ut[MATNR]==$lote[MATNR] && $ut[CHARG]==$lote[CHARG]){
								echo  "DOCMAT_HU;sl-utr;".";".$ut[MATNR].";".$ut[CHARG].";".$ut[EXIDV].";".number_format($ut[VEMNG], 3, '.', '').";\r\n";						
							}												
						}
					}
				}
...
//								$cant_lote = $cant_lote + ($ut[VEMNG] * $linea[SIGNO]);
	...
	$aux2_ut_mov_mat = array();	
	
	foreach ($cabecera_mov_mat as $cabecera){
		$cabMovMat = new CabeceraMovimientoMaterial();
		$cabMovMat_anteriror = new CabeceraMovimientoMaterial();
		$cabMovMat_anteriror = $cabMovMat_anteriror->buscarExtendido($cabecera[MAT_DOC],$cabecera[DOC_YEAR]);
		$tipo_cambio_cabecera = "SINCAMBIOS";
		if (!$cabMovMat_anteriror->existe($cabecera[MAT_DOC],$cabecera[DOC_YEAR])){
			foreach ($cabecera as $k =>$v){
				$cabMovMat->$k = $cabecera[$k];
			}		
			$cabMovMat->TIPO_MOV="MOVIMIENTO";
			$cabMovMat->save();
			$tipo_cambio_cabecera = "ALTA";
		}else{
			$cabMovMat = $cabMovMat_anteriror;
			foreach ($cabecera as $k =>$v){
				$cabMovMat->$k = $cabecera[$k];
			}		
			foreach ($cabecera as $k =>$v){
				if($cabMovMat->$k!=$cabMovMat_anteriror->$k){
					$tipo_cambio_cabecera = "MODIFICACION";
				}
			}		
			$cabMovMat->TIPO_MOV="MOVIMIENTO";
			$cabMovMat->save();
		}
		$cabecera[TIPO_ACTU]=$tipo_cambio_cabecera;
		$cabecera[TIPO_MOV]='MOVIMIENTO';
	...
	foreach ($aux_linea_mov_mat as $linea){
		...
		$linMovMat_anteriror = new LineaMovimientoMaterial();
		$linMovMat_anteriror = $linMovMat_anteriror->buscarExtendido($linea[MAT_DOC],$linea[DOC_YEAR],$linea[MATNR]);
		$tipo_cambio_linea = "SINCAMBIOS";
		if (!$linMovMat_anteriror->existe($linea[MAT_DOC],$linea[DOC_YEAR],$linea[MATNR])){
			foreach ($linea as $k =>$v){
				$linMovMat->$k = $linea[$k];
			}		
			$linMovMat->save();
			$tipo_cambio_linea = "ALTA";
		}else{
			$linMovMat = $linMovMat_anteriror;
			foreach ($linea as $k =>$v){
				$linMovMat->$k = $linea[$k];
			}		
			foreach ($linea as $k =>$v){
				if($linMovMat->$k!=$linMovMat_anteriror->$k){
					$tipo_cambio_linea = "MODIFICACION";
				}
			}		
			$linMovMat->save();
		}
		$linea[TIPO_ACTU]=$tipo_cambio_linea;
...
//				echo "DOCMAT_HU;st-lar;".$tipo_cambio.";".$linea[MATNR].";".$linea[VEMNG].";".$linea[VEMEH].";\r\n";
	foreach ($aux_lote_mov_mat as $lote){
		...
		$lotMovMat_anteriror = new LoteMovimientoMaterial();
		$lotMovMat_anteriror = $lotMovMat_anteriror->buscarExtendido($lote[MAT_DOC],$lote[DOC_YEAR],$lote[MATNR],$lote[CHARG]);
		$tipo_cambio_lote = "SINCAMBIOS";
		if (!$lotMovMat_anteriror->existe($lote[MAT_DOC],$lote[DOC_YEAR],$lote[MATNR],$lote[CHARG])){
			foreach ($lote as $k =>$v){
				$lotMovMat->$k = $lote[$k];
			}		
			$lotMovMat->save();
			$tipo_cambio_lote = "ALTA";
		}else{
			$lotMovMat = $lotMovMat_anteriror;
			foreach ($lote as $k =>$v){
				$lotMovMat->$k = $lote[$k];
			}		
			foreach ($lote as $k =>$v){
				if($lotMovMat->$k!=$lotMovMat_anteriror->$k){
					$tipo_cambio_lote = "MODIFICACION";
				}
			}		
			$lotMovMat->save();
		}									
		$lote[TIPO_ACTU]=$tipo_cambio_lote;	
	...
	foreach ($ut_mov_mat as $ut){
		...
		$utMovMat_anteriror = new HUMovimientoMaterial();
		$utMovMat_anteriror = $utMovMat_anteriror->buscarExtendido($ut[MAT_DOC],$ut[DOC_YEAR],$ut[MATNR],$ut[CHARG],$ut[EXIDV]);
		$tipo_cambio_ut = "SINCAMBIOS";
		if (!$utMovMat_anteriror->existe($ut[MAT_DOC],$ut[DOC_YEAR],$ut[MATNR],$ut[CHARG],$ut[EXIDV])){
			foreach ($ut as $k =>$v){
				$utMovMat->$k = $ut[$k];
			}		
			$utMovMat->save();
			...
			$utMovMat = $utMovMat_anteriror;
			foreach ($ut as $k =>$v){
				$utMovMat->$k = $ut[$k];
			}		
			foreach ($ut as $k =>$v){
				if($utMovMat->$k!=$utMovMat_anteriror->$k){
					$tipo_cambio_ut = "MODIFICACION";
				}
			}		
			$utMovMat->save();
		}																	
		$ut[TIPO_ACTU]=$tipo_cambio_ut;
	...
	$aux2_cabecera_mov_mat = $aux2_cabecera_mov_mat->FindAll("CabeceraMovimientoMaterial", "TIPO_MOV='MOVIMIENTO' AND Enviado_Anita = ''");

	foreach ($aux2_cabecera_mov_mat as $cabecera){
		foreach ($aux2_linea_mov_mat as $linea){
			if($linea[MAT_DOC]==$cabecera->MAT_DOC && $linea[DOC_YEAR]==$cabecera->DOC_YEAR){
				if ($linea[TIPO_ACTU]=="ALTA"){
					...
					fwrite($file_paquete,"DOCMAT_HU;st-lar;".$cabecera->MAT_DOC.";".$cabecera->DOC_YEAR.";".$cabecera->PSTNG_DATE.";".$linea[MOVE_TYPE].";".$linea[MATNR].";".number_format($linea[VEMNG], 3, '.', '').";".$linea[VEMEH].";\r\n");
					foreach ($aux2_lote_mov_mat as $lote){
						if($lote[MAT_DOC]==$linea[MAT_DOC] && $lote[DOC_YEAR]==$linea[DOC_YEAR] && $lote[MATNR]==$linea[MATNR]){					
							fwrite($file_paquete, "DOCMAT_HU;st-rld;".$linea[MATNR].";".$lote[CHARG].";".number_format($lote[VEMNG], 3, '.', '').";".$lote[VEMEH].";".$lote[VFDAT].";\r\n");
							foreach ($aux2_ut_mov_mat as $ut){
								if($ut[MAT_DOC]==$lote[MAT_DOC] && $ut[DOC_YEAR]==$lote[DOC_YEAR] && $ut[MATNR]==$lote[MATNR] && $ut[CHARG]==$lote[CHARG]){
									fwrite($file_paquete, "DOCMAT_HU;sl-utr;".$linea[MATNR].";".$lote[CHARG].";".$ut[EXIDV].";".number_format($ut[VEMNG], 3, '.', '').";\r\n");						
								}												
							}
						}
					}
