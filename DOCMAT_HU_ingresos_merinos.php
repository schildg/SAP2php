...
//	saprfc_table_append ($fce,"MATERIAL_RA", array ("SIGN"=>"I","OPTION"=>"BT","LOW"=>"000000000800000226","HIGH"=>"000000000800000226"));
	...
	saprfc_table_append ($fce,"MOVE_TYPE_RA", array ("SIGN"=>"I","OPTION"=>"EQ","LOW"=>"101","HIGH"=>"101"));
	...
	saprfc_table_append ($fce,"PLANT_RA", array ("SIGN"=>"I","OPTION"=>"EQ","LOW"=>"2001","HIGH"=>"2001"));
	saprfc_table_append ($fce,"PLANT_RA", array ("SIGN"=>"I","OPTION"=>"EQ","LOW"=>"2002","HIGH"=>"2002"));
	...
	saprfc_table_append ($fce,"STGE_LOC_RA", array ("SIGN"=>"I","OPTION"=>"EQ","LOW"=>"AC01","HIGH"=>"AC01"));
	saprfc_table_append ($fce,"STGE_LOC_RA", array ("SIGN"=>"I","OPTION"=>"EQ","LOW"=>"AC02","HIGH"=>"AC02"));
	saprfc_table_append ($fce,"STGE_LOC_RA", array ("SIGN"=>"I","OPTION"=>"EQ","LOW"=>"ACFP","HIGH"=>"ACFP"));
	...
	foreach ($aux2_cabecera_mov_mat as $cabecera){
		foreach ($aux2_linea_mov_mat as $linea){
			if($linea[MAT_DOC]==$cabecera->MAT_DOC && $linea[DOC_YEAR]==$cabecera->DOC_YEAR){
				...
				fwrite($file_paquete,"MER_MUEST;st-lar;".$cabecera->MAT_DOC.";".$cabecera->DOC_YEAR.";".$cabecera->PSTNG_DATE.";".$linea[MOVE_TYPE].";".$linea[MATNR].";".number_format($linea[VEMNG], 3, '.', '').";".$linea[VEMEH].";\r\n");
				foreach ($aux2_lote_mov_mat as $lote){
					if($lote[MAT_DOC]==$linea[MAT_DOC] && $lote[DOC_YEAR]==$linea[DOC_YEAR] && $lote[MATNR]==$linea[MATNR]){					
						fwrite($file_paquete, "MER_MUEST;st-rld;".$linea[MATNR].";".$lote[CHARG].";".number_format($lote[VEMNG], 3, '.', '').";".$lote[VEMEH].";".$lote[VFDAT].";\r\n");
						foreach ($aux2_ut_mov_mat as $ut){
							if($ut[MAT_DOC]==$lote[MAT_DOC] && $ut[DOC_YEAR]==$lote[DOC_YEAR] && $ut[MATNR]==$lote[MATNR] && $ut[CHARG]==$lote[CHARG]){
								fwrite($file_paquete, "MER_MUEST;sl-utr;".$linea[MATNR].";".$lote[CHARG].";".$ut[EXIDV].";".number_format($ut[VEMNG], 3, '.', '').";\r\n");						
							}												
						}
					}
				}
