...
class Deuda_CRM_TODO extends SuperClase{
	function Deuda_CRM_TODO() {
		$this->CLASE_OBJETO='Deuda_CRM_TODO';
	}
	function leyenda() {
		return $this->id;
	}
	...
	function save(){
		if($this->id){
			$ind_deudores = New Deuda_CRM_TMP();
			$ind_deudores = MyActiveRecord :: FindFirst("Deuda_CRM_TMP","BUKRS = '$this->BUKRS' AND BELNR = '$this->BELNR' AND GJAHR = '$this->GJAHR'");		
			...
			if($cab_factura){// SI ENTRA ES PORQUE VIENE DE LA FACTURA SD, SINO ES UN DOCUMENTO VIEJO CARGDADO EN DEUDA_CRM
				...
				$ind_deudores = New Deuda_CRM_TMP();
				$ind_deudores = MyActiveRecord :: FindFirst("Deuda_CRM_TMP","BUKRS = '$doc_contable->BUKRS' AND BELNR = '$doc_contable->BELNR' AND GJAHR = '$doc_contable->GJAHR'");		
